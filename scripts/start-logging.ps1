...
Write-Output "WFT A"
$bspid = adb wait-for-device shell pidof com.beatgames.beatsaber
...
if ($bspid) {
    Write-Host "PID found: $bspid"
} else {
    Write-Host "Timed out after $timeout seconds with no PID found."
}

Write-Output "WFT B $bspid"
$command = "adb logcat "
